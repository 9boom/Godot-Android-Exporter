...
ANDROID_BANNER = f""" {Fore.GREEN}
     ...
     GAE NEWS:
              This is the last version update of this program
              Because the new versions of godot editor
              has been supported an 'export APK' in mobile.
"""
...
def run_command(command, error_message):
    ...
    try:
        ...
        spinner.stop()
        print_color(f"\r*Command executed: {' '.join(command)}")
        return result
    except subprocess.CalledProcessError as e:
        spinner.stop()
        print_color(f"\r{error_message}: {e}", Fore.RED)
        print_color(f"{e.stderr}", Fore.YELLOW)
        raise
...
def show_banner():
    ...
    print_color("Godot Android Exporter Toolkit v2.0", Fore.MAGENTA)

def decompile_apk(apk_path, decompiled_dir):
    print_color("Prepairing a template...",Fore.CYAN)
    run_command(["apktool", "d", "-f", "-o", decompiled_dir, apk_path], "Decompilation failed")

def read_properties():
    print_color("Reading properties.gae...", Fore.CYAN)
    ...
    for key in config['DEFAULT']:
        properties_dict[key] = config['DEFAULT'][key]
        print_color(f"{key}: {config['DEFAULT'][key]}")

    permissions_str = properties_dict.get('permissions', '')
    permissions_list = [p.strip() for p in permissions_str.split(',') if p.strip()]
    properties_dict['permissions_list'] = permissions_list
    ...
    if permissions_list:
        ...
        print_color("No additional permissions specified")

    return properties_dict

def update_app_name(decompiled_dir, app_name):
    print_color("Updating app name...", Fore.CYAN)
    ...
    for values_dir in values_dirs:
        ...
        print_color(f"Processing: {strings_path}")
        tree = ET.parse(strings_path)
        root = tree.getroot()

        for string_elem in root.findall('string'):
            if string_elem.get('name') == 'godot_project_name_string':
                print_color(f"Converting app name {string_elem.text} to {app_name}")
...
def update_version_info(decompiled_dir, version_code, version_name):
    print_color("Updating version...", Fore.CYAN)
    ...
    if ET.QName(ns['android'], 'versionName') in root.attrib:
        print_color(f"Updating versionName: {root.attrib[ET.QName(ns['android'], 'versionName')]} -> {version_name}")
    root.attrib[ET.QName(ns['android'], 'versionName')] = version_name

    tree.write(manifest_path, encoding='utf-8', xml_declaration=True)
    print_color(f"Updated versions: versionName={version_name}, versionCode={version_code}")

def update_package_name(decompiled_dir, new_package):
    print_color("Updating packages name...", Fore.CYAN)
    manifest_path = os.path.join(decompiled_dir, "AndroidManifest.xml")
    ...
    with open(manifest_path, 'r+', encoding='utf-8') as f:
        ...
        if content != replaced_content:
            print_color(f"Replaced package: com.nineboom.AGT to {new_package}")
            f.seek(0)
...
def update_sdk_versions(decompiled_dir, min_sdk, target_sdk):
    print_color("Updating sdk version...", Fore.CYAN)
    manifest_path = os.path.join(decompiled_dir, "AndroidManifest.xml")
    tree = ET.parse(manifest_path)
    root = tree.getroot()
    ns = {'android': 'http://schemas.android.com/apk/res/android'}
    ...
    if uses_sdk is None:
        ...
        print_color("Created new uses-sdk element")

    uses_sdk.set(ET.QName(ns['android'], 'minSdkVersion'), min_sdk)
    uses_sdk.set(ET.QName(ns['android'], 'targetSdkVersion'), target_sdk)

    tree.write(manifest_path, encoding='utf-8', xml_declaration=True)
    print_color(f"Updated SDK versions: minSdk={min_sdk}, targetSdk={target_sdk}")

def update_permissions(decompiled_dir, permissions_list):
    print_color("Updating permissions...", Fore.CYAN)
    manifest_path = os.path.join(decompiled_dir, "AndroidManifest.xml")
    tree = ET.parse(manifest_path)
    root = tree.getroot()
    ns = {'android': 'http://schemas.android.com/apk/res/android'}

    # ลบ permissions เก่าออกทั้งหมด
    for elem in root.findall('uses-permission'):
        name_attr = elem.get(ET.QName(ns['android'], 'name'))
        if name_attr and name_attr.startswith('android.permission'):
            print_color(f"Removing old permission: {name_attr}")
            root.remove(elem)

    # เพิ่ม permissions ใหม่
    for perm in permissions_list:
        if perm.strip():
            print_color(f"Adding permission: {perm}")
            new_perm = ET.SubElement(root, 'uses-permission')
            new_perm.set(ET.QName(ns['android'], 'name'), perm.strip())

    tree.write(manifest_path, encoding='utf-8', xml_declaration=True)
    print_color(f"Updated {len(permissions_list)} permissions")
...
def replace_icons(decompiled_dir, icon_path):
    print_color("Replacing icons...", Fore.CYAN)
    try:
        ...
        for mipmap_dir in mipmap_dirs:
            ...
            print_color(f"Processing {os.path.basename(mipmap_dir)} - Size: {size[0]}x{size[1]}")
            ...
            for icon_file in ['icon.png', 'icon_background.png', 'icon_foreground.png']:
                ...
                if os.path.exists(target_path):
                    ...
                    print_color(f"Processing: {target_path}")
...
def check_and_copy_cl_file(assets_dir):
    print_color("Adjusting app screen appropriate...", Fore.CYAN)
    ...
    if not os.path.exists(cl_dest):
        if os.path.exists(cl_source):
            ...
            print_color("Copied _cl_ file to assets")
        else:
            print_color("_cl_ file not found in script directory!", Fore.YELLOW)
    else:
        print_color("_cl_ file already exists in assets", Fore.GREEN)

def rebuild_and_sign(decompiled_dir, properties):
    print_color("Building app & signing apk...", Fore.CYAN)
    ...
    sign_cmd = [
        "apksigner", "sign",
        "--ks", properties['keystore_path'],
        "--ks-key-alias", properties['keystore_user'],
        "--ks-pass", f"pass:{properties['keystore_pass']}",
        "--key-pass", f"pass:{properties['keystore_pass']}",
        aligned_apk
    ]
    run_command(sign_cmd, "Signing failed")
    os.rename(aligned_apk, signed_apk)
    print_color(f"\n{Fore.GREEN}Completed successfully", Fore.GREEN)
    print_color(f"Final APK: {signed_apk}", Fore.CYAN)
    ask_remove_junk = input('Do you want to remove unnecessary junk files? (Will remove: aligned.apk.idsig, decompiled_apk, unsigned.apk) [Y:Yes/N:No]: ')
    if ask_remove_junk.lower() == 'y':
        if os.path.exists('aligned.apk.idsig'):
            os.remove('aligned.apk.idsig')
        if os.path.exists(decompiled_dir):
            shutil.rmtree(decompiled_dir)
        if os.path.exists('unsigned.apk'):
            os.remove('unsigned.apk')
        print_color("Temporary files removed", Fore.GREEN)
    else:
        print_color("Keeping temporary files", Fore.YELLOW)
...
def main():
    ...
    try:
        ...
        # เรียกใช้ฟังก์ชันจัดการ permissions ใหม่
        if 'permissions_list' in properties:
            update_permissions(decompiled_dir, properties['permissions_list'])
        ...
        if 'project_zip_path' in properties:
            print_color("Extracting zip...", Fore.CYAN)
            with zipfile.ZipFile(properties['project_zip_path'], 'r') as zip_ref:
                zip_ref.extractall(assets_dir)
                print_color(f"Extracted to: {assets_dir}")
